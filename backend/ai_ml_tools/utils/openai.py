...
# Mock classes that mimic the real ones but don't require credentials
class MockAzureOpenAI:
    ...
    class ChatCompletions:
        @staticmethod
        def create(model=None, messages=None, temperature=None, frequency_penalty=None, presence_penalty=None, stop=None, stream=None):
                    ...
                    self.choices = [
                        type('obj', (object,), {
                            'message': type('obj', (object,), {
                                'role': 'assistant',
                                'content': f"Mock response to: {messages[-1]['content'] if messages else 'no input'}"
                            }),
                            'finish_reason': 'stop'
                        })
                    ]
...
class MockAzureOpenAIEmbeddings:
    def __init__(self, model=None, azure_endpoint=None, api_key=None, openai_api_version=None):
        self.model = model
...
# Create mock clients
openAIClient_o3_mini = MockAzureOpenAI()
openAIClient_4o = MockAzureOpenAI()
openAIClient_4o_mini = MockAzureOpenAI()

# Mock embeddings object
embeddings = MockAzureOpenAIEmbeddings()

def num_tokens_from_string(string) -> int:
    # Simple approximation
    return len(string) // 4

def get_relevent_chunks(chat_history, document_chunks, document_metadata):
    document_content = 'Mock document content for testing'
    return document_content

def request_openai_response(question, conversation_input, model="gpt-4o-mini", tempurature=0.3):
    print("Using MOCK OpenAI implementation!")
    conversation_input.append({"role": "user", "content": question})
    ...
    tokens_used = len(question) // 3
    
    return [conversation_input, response_text, tokens_used]
